...
void phase_update(Rubik &r){
    int prev_phase;
    while(1){
        prev_phase=r.phase;
        if(r.phase==global_phase)
            break;
        r.phase_check();
...
vector<Rubik> one_point_crossover(Rubik &p1, Rubik &p2){
    // return two children generated by p1 and p2
    vector<Rubik> ret;
    uniform_real_distribution<> dis0(0.0, 1.0);

    string a = p1.current_state_actions;
    string b = p2.current_state_actions;

    if (dis0(gen) > crossover_rate || a.empty() || b.empty()){
        ret.emplace_back(p1);
        ret.emplace_back(p2);
        return ret;
    }
